...
| 训练 | **运行中** — 从原始最佳checkpoint step 11,000恢复，新分支快照至step 15,465 |
| 上下文 | 4K / 8K / 16K / 32K / 64K（128K 因40GB显存上限未纳入v1） |
| 新学习率 | 从step 11,001起每一步恒定2e-5；无warmup、无衰减、无自动重启、无开发集回退 |
| 开发集 MLM | 新分支已有8次评估；最佳1.220370 @ 11,500，最新1.221003 @ 15,000；越低越好 |
| 活跃语料覆盖 | 121.6亿 / 1142亿唯一tokens（10.65%）；采样状态随checkpoint回退，cycle 0、无受控复用；不是训练完成度 |
| Checkpoint | 新分支已永久保存8份完整checkpoint，最新step 15,000；本次重算文件SHA-256通过 |
| GPU | gpu10的GPU 0/1/2，均为A100 40GB；本次核查三卡瞬时100%，本任务显存约31GiB/卡；GPU0另有他人计算进程，总占用约35GiB，未干预 |
...
## 训练进度（step 11,000最佳点重启分支，快照至step 15,465）

状态核查：2026-09-05 23:24（北京时间）。图表固定在step 15,465，固定开发集最新评估为step 15,000；实时训练会继续前进。

![训练曲线 step 15465](docs/brassicaceae_genomefm_v1_training_curves_step15465.png)

[下载PDF](docs/brassicaceae_genomefm_v1_training_curves_step15465.pdf) · [机器可读摘要](docs/training_summary_v1.json)

### 白话解读

**A. 全部上下文的训练 MLM（左上）**
横轴是优化步数，纵轴是掩码预测损失（MLM，越低越好）；浅蓝点为抽样单步值，红线为100步滚动中位数。权威曲线只拼接原始训练step 1–11,000和新分支step 11,001以后，不混入废弃分支。新分支已运行4,465步，最近50步训练MLM均值0.997，没有持续发散迹象。为看清后期趋势，纵轴省略了部分早期高损失峰值；训练损失不能与开发集损失直接比较。

**B. 五档上下文（右上）**
横轴仍是优化步数，五条颜色曲线分别表示4K–64K上下文，每条用该长度最近50个点的滚动中位数平滑。图例的最近50点均值为4K 1.008、8K 1.012、16K 1.001、32K 1.008、64K 1.008。五档均已在新分支实际运行，未见某一长度持续异常；不同长度样本组成不同，不能把较低训练损失直接解释为长程能力更强。纵轴同样聚焦后期范围。
...
**D. 开发集（右下）**
横轴是每500步一次的固定开发集评估，纵轴是损失；红色为MLM，绿色和紫色分别为区域分类与阅读框预测。黑圈标记当前最低MLM：新分支step 11,500的1.220370；最新step 15,000为1.221003。下表保留六位小数，避免总览图把微小变化压平。父checkpoint step 11,000的1.220776只作为恢复起点，不冒充新分支结果。
...
| 14,000 | 1.221015 | 新分支 |
| 14,500 | 1.220489 | 新分支 |
| 15,000 | 1.221003 | 新分支，最新评估 |

### 当前判断

- **恢复闭环**：step 11,000的模型、812项AdamW状态、五档sampler cursor、pair cursor、token账本和3份rank RNG均完整恢复。
- **运行层面健康**：新分支已连续运行4,465步，每一步学习率均为2e-5；3个FSDP rank存活，核查时三张A100满载，没有失败标记，cycle 0内复用tokens为0。
- **已回到原先较低损失水平，但没有持续下降**：8个开发集点均值1.220801，范围1.220370–1.221426；最佳比父checkpoint降低0.000405（约0.0332%），属于很小的数值改善。最新值比父checkpoint高0.000227，不能说当前参数已经稳定优于父checkpoint。
- **checkpoint完整性**：8份manifest/receipt及文件大小核对通过，最新step 15,000的完整训练状态文件SHA-256重新计算通过；每500步永久保存策略未变。
...
2. 新分支出现略低于1.220776的开发集最低值，但8次评估整体围绕原有水平波动；不能把一个最低点当成持续改善或下游提升。
