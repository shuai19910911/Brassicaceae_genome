...
当前全部任务状态为 `DESIGNED / NOT EXECUTED`：尚未冻结任务数据、split、模型预测或结果。本文不能被引用为“评测已完成”。
...
当前还没有下游数据release，下表是进入确认性主分析的预注册下限，不是实际样本数。任何任务低于下限，在预训练release前标记`INFEASIBLE_FOR_CONFIRMATORY`；不能用更多重叠碱基或proxy标签替代独立生物单位。
